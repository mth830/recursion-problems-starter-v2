...
function greedyMakeChange(target, coins = [25, 10, 5, 1], curr = []) {
  ...
  for (let i = 0; i < coins.length; i++) {
    let coin = coins[i];
    curr.push(coin)
    if (greedyMakeChange(target - coin, coins, curr)) {
      return curr;
    }
    curr.pop();
  }
...
function makeBetterChange(target, coins = [25, 10, 5, 1], curr = []) {
  ...
  for (let i = 0; i < coins.length; i++) {
    let coin = coins[i];
    curr.push(coin)
    let possibility = makeBetterChange(target - coin, coins.slice(i), curr)
    if (possibility && answer === null || (Array.isArray(possibility) && possibility.length < answer.length)) {
      answer = possibility.slice();
    }
...
/*function stackTest(num) {
  if (num === 0) return [num];
  return stackTest(num - 1);
}
console.log(stackTest(5))*/

//console.log(Array.isArray(false))
//console.log(makeBetterChange(1))
